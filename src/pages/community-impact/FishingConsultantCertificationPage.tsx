...
const WRITTEN_SECTIONS = [
  {
    ...
    icon: <Fish size={13} />,
    colour: 'border-hair bg-blue-500/5',
  ...
  },
  {
    ...
    colour: 'border-hair bg-red-500/5',
  ...
  },
  {
    ...
    icon: <Waves size={13} />,
    colour: 'border-hair bg-cyan-500/5',
...
const scoreLabel = (s: number | null) => {
  ...
  if (s === 3)    return { text: 'Advanced',     color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-hair' };
  if (s === 2)    return { text: 'Proficient',   color: 'text-blue-400',    bg: 'bg-blue-500/10',    border: 'border-hair'    };
  if (s === 1)    return { text: 'Emerging',     color: 'text-amber-400',   bg: 'bg-amber-500/10',   border: 'border-hair'   };
  return               { text: 'No Evidence',  color: 'text-red-400',     bg: 'bg-red-500/10',     border: 'border-hair'     };
};
...
const FishingConsultantCertificationPage: React.FC = () => {
  ...
  const speak = useCallback((text: string) => {
    ...
    void playPidginVoice(text.slice(0, 300), 'english', {
      ...
      onError: (err) => {
        console.warn('[FishingConsultantCertificationPage] SpeechGen TTS failed, falling back to browser voice:', err);
        speakBrowser(text);
      },
    });
...
Return valid JSON only (no markdown, no code fences):
{
  ...
  "main_growth_area": "1-2 sentences on the clearest area for improvement with a specific suggestion"
      ...
      // fishing_cert_evaluation column saveToDashboard just wrote — see
      ...
      if (dashboardRowId && user?.id) {
        saveEvaluation(user.id, {
          ...
          activityType: 'fishing_consultant_certification',
    ...
    return (
      <div className="min-h-screen bg-paper flex flex-col">
        ...
        <div className="flex-1 flex flex-col max-w-3xl mx-auto w-full px-4 py-4">
          ...
          <div className="px-4 py-2.5 bg-surface border border-hair rounded-xl mb-3 flex items-center gap-2">
        ...
        <main className="flex-1 overflow-y-auto relative z-10">
          <div className="max-w-2xl mx-auto px-6 py-8 space-y-5">
            ...
            <div className="p-6 bg-surface border border-hair rounded-2xl text-center space-y-2">
              <div className="w-16 h-16 mx-auto rounded-2xl bg-accent hover:bg-accent/90 flex items-center justify-center">
                <Fish size={32} className="text-white" />
              </div>
              <h1 className="text-2xl font-bold text-ink">Fishing Consultant Certification</h1>
              ...
              <div className="space-y-2">
                {RUBRIC_DIMENSIONS.map(d => (
                  <div key={d.id} className="flex items-start gap-2 text-sm">
                    <Fish size={12} className="text-accent flex-shrink-0 mt-0.5" />
                    <div>
          ...
          <div className="max-w-2xl mx-auto px-6 py-6 space-y-4">
                ...
                {WRITTEN_SECTIONS.map(sec => {
                  ...
                  return (
                    <div key={sec.key} className={`rounded-2xl border ${sec.colour} p-5 space-y-3`}>
                      ...
                      <div className="flex items-start gap-2 p-3 bg-amber-500/10 border border-hair rounded-xl">
                      ...
                      <textarea value={val} onChange={e => setWritten(sec.key, e.target.value)}
                        ...
                        className="w-full bg-card border border-hair rounded-xl px-4 py-3 text-sm text-body placeholder-muted outline-none focus:border-accent resize-none leading-relaxed"
                      />
            ...
            {buildTab === 'sessions' && (
              <div className="space-y-4">
                  ...
                  {FISHER_PERSONAS.map(persona => {
                    ...
                    return (
                      <button key={persona.id} onClick={() => startSession(persona)}
                        className={`w-full text-left flex items-start gap-3 p-4 rounded-xl border transition-all ${done ? 'border-hair bg-surface opacity-75' : 'border-hair bg-paper hover:border-hair hover:bg-paper'}`}>
                ...
                {evalError && (
                  <div className="p-4 bg-red-500/15 border border-hair rounded-xl text-red-300 flex gap-2 text-sm">
        ...
        <main className="flex-1 overflow-y-auto relative z-10">
          <div className="max-w-2xl mx-auto px-6 py-6 space-y-5">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {allProficient && (
                <button onClick={() => setView('certificate')}
                  className="flex items-center gap-2 px-5 py-2.5 bg-accent hover:bg-accent/90 hover:opacity-90 text-white text-sm font-bold rounded-xl transition-colors">
      ...
      <div className="min-h-screen bg-paper flex flex-col">
        ...
        <main className="flex-1 overflow-y-auto relative z-10">
          <div className="max-w-2xl mx-auto px-6 py-6 space-y-5">
            ...
            {!allProficient && (
              <div className="p-4 bg-amber-500/15 border border-hair rounded-xl text-amber-300 flex gap-2 text-sm">
            ...
            {allProficient && (
              <>
                <div className="p-6 bg-surface border border-hair rounded-2xl text-center space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    {assessmentScores.map(sc => {
                      ...
                      return (
                        <div key={sc.assessment_name} className="flex items-center justify-between px-3 py-1.5 bg-paper rounded-lg">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-center text-xs text-muted">{`Cyan-themed PDF · ${branding.institutionName}`}</p>
                </div>
              </>
            )}
