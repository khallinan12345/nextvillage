...
Return valid JSON only (no markdown, no code fences):
{
  ...
  "main_growth_area": "1-2 sentences on the clearest area for improvement with a specific suggestion"
      ...
      // agriculture_cert_evaluation column saveToDashboard just wrote — see
      ...
      if (dashboardRowId && user?.id) {
        saveEvaluation(user.id, {
          ...
          activityType: 'agriculture_consultant_certification',
              ...
              {sessionMessages.map(msg => (
                <div key={msg.id} className={`flex items-start gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <div className={`max-w-[78%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${msg.role === 'user' ? 'bg-card border border-hair text-body rounded-tr-sm' : 'bg-paper text-body rounded-tl-sm'}`}>
                    <p className="text-[10px] font-bold mb-1 text-muted">{msg.role === 'user' ? 'You (Consultant)' : activePersona.name}</p>
                    {msg.content}
      ...
      <div className="min-h-screen flex flex-col bg-paper">
        ...
        <main className="flex-1 overflow-y-auto relative z-10">
          <div className="max-w-2xl mx-auto px-6 py-8 space-y-5">
            ...
            {/* Header */}
            <div className="p-6 bg-gradient-to-br bg-surface border border-hair rounded-2xl text-center space-y-2">
              <div className="w-16 h-16 mx-auto rounded-2xl bg-gradient-to-br bg-accent flex items-center justify-center">
                <Sprout size={32} className="text-white" />
              </div>
              <h1 className="text-2xl font-bold text-ink">Agriculture Consultant Certification</h1>
              <p className="text-sm text-muted">Community Impact Track · Davidson AI Innovation Center</p>
                ...
                {[
                  ...
                  <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-paper">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className={`text-xs font-semibold mt-1 ${item.done ? 'text-green-400' : 'text-muted'}`}>{item.progress}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {assessmentScores.length > 0 && (
              <button onClick={() => setView('results')}
                className="w-full py-2 text-xs text-green-400 hover:text-muted transition-colors underline">
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="max-w-2xl mx-auto px-6 py-6 space-y-4">
            ...
            <div className="flex items-center justify-between">
              <button onClick={() => setView('overview')} className="flex items-center gap-1.5 text-sm text-muted hover:text-ink transition-colors">
                ← Overview
              </button>
                  ...
                  return (
                    <div key={sec.key} className={`rounded-2xl border ${sec.colour} p-5 space-y-3`}>
                      ...
                      <textarea
                        ...
                        className="w-full bg-card border border-hair border border-hair rounded-xl px-4 py-3 text-sm text-body placeholder-muted outline-none focus:border-accent resize-none leading-relaxed"
                      />
      ...
      <div className="min-h-screen flex flex-col bg-paper">
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="max-w-2xl mx-auto px-6 py-6 space-y-5">
            ...
            {allProficient && (
              <>
                <div className="p-6 bg-gradient-to-br bg-surface border border-hair rounded-2xl text-center space-y-4">
