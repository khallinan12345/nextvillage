// api/ai900-task-instruction.ts
// Handles three modes for AI-900 cert prep:
...
async function callClaude(system: string, user: string, maxTokens = 1200, action = 'generate'): Promise<string> {
  ...
  if (!res.ok) {
    ...
    console.error('[ai900-task-instruction] Anthropic error:', res.status, err);
...
async function generateInstruction(body: any): Promise<object> {
  ...
  const system = `You are an AI-900 certification coach for young women in Oloibiri, Bayelsa State, Nigeria, learning through the Girls AIing & Vibing platform at the Davidson AI Innovation Center.

Your role: generate Socratic teaching questions — not explanations. Make the learner think and articulate understanding in their own words. Every example must connect to their lived context: creek fishing, cassava farming, the OWFA, oil contamination history, community radio, the Girls AIing platform itself.
...
- subTasks: Socratic questions grounded in Oloibiri / Girls AIing context — never generic
...
- No bullet lists inside JSON string values — use plain prose`;
  ...
  const user = `Generate a TaskInstruction for AI-900 topic: "${topicLabel}" (Domain ${domain}, id: ${topicId}).
...
async function evaluateAnswer(body: any): Promise<object> {
  ...
  const system = `You are an AI-900 certification coach evaluating a learner's answer. Be warm, precise, and honest.
...
Never give away the full answer — coach toward understanding.`;
  ...
  const user = `AI-900 topic: "${topicId}" (Domain ${domain})
...
async function generateHint(body: any): Promise<object> {
  ...
  const system = `You are an AI-900 certification coach giving a learner a nudge — not the answer.
...
Rules: do not confirm or deny specific parts of their answer. Point toward a concept or ask a guiding question.`;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.log(`[ai900-task-instruction] mode=${mode ?? 'generate'}, topic=${body.topicId}`);
    ...
    console.error('[ai900-task-instruction] Error:', err);
